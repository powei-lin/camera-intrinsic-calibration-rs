...
use camera_intrinsic_calibration::detected_points::{FrameFeature, FeaturePoint};
use camera_intrinsic_model::{GenericModel, UCM, EUCM};
...
fn test_find_best_two_frames() {
    ...
    let make_frame = |n: usize| -> FrameFeature {
        ...
        for i in 0..n {
            features.insert(i as u32, FeaturePoint {
                p2d: Vec2::ZERO,
                p3d: glam::Vec3::ZERO,
            });
        }
...
fn test_convert_model() {
    ...
    // Target EUCM
    let mut eucm = GenericModel::EUCM(EUCM::new(&na::dvector![400.0, 400.0, 320.0, 240.0, 0.0, 1.0], 640, 480));
